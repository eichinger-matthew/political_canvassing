...
sim_neighbors <- function(df, num_residents, lower_lim, upper_lim){
  ...
  anchor_offsets <- 
    matrix(data = runif(n = 2*num_residents, min = lower_lim, max = upper_lim),
...
while(nrow(been_visited) > sum(been_visited$visited) |
  ...
  # if there exist match winners, continue
